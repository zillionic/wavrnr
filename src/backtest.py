...
def main() -> None:
    ...
    if strategy.buy_count > 0:
        avg_price = strategy.avg_entry_price
        target_price = avg_price * (1 + TAKE_PROFIT_PCT)
        stop_price = avg_price * (1 - STOP_LOSS_PCT)
        print(
            f"\n현재 미청산 포지션: {strategy.buy_count}회 매수, "
            f"마지막 매수가 {strategy.last_buy_price}, 평단가 {avg_price:.2f}, "
            f"목표 매도가(평단가 +{TAKE_PROFIT_PCT * 100:.1f}%) {target_price:.2f}, "
            f"손절가(평단가 -{STOP_LOSS_PCT * 100:.1f}%) {stop_price:.2f}"
        )
